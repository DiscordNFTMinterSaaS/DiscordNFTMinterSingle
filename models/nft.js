...
const nftModel = db.getModel("nft");
const mintErrLogModel = db.getModel("mintErrLog");

const { JSON_RPC_PROVIDER, WALLET_PRIVATE_KEY, CONTRACT_ADDRESS, TX_DOMAIN, OPENSEA_PREFIX, NETWORK_NAME } = process.env;
...
exports.mint = async ({ userId, metaUrl, walletAddress, name, guildId, channelId }) => {
  const rz = await nftModel.create({
    userId, metaUrl, name, guildId, channelId, NETWORK_NAME, CONTRACT_ADDRESS
  });
  ...
  try {
    ...
    await nftModel.findByIdAndUpdate(_id, { $set: { transactionResponse } });
...
exports.getMintResult = async ({ _id, hash }) => {
  ...
  await nftModel.findByIdAndUpdate(_id, { $set: { transactionReceipt } });
